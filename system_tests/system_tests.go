	...
	Context("Persistence", func() {
		...
		It("persists messages after pod deletion", func() {
			By("publishing a message", func() {
				err := publishToNewQueue(hostname, port, username, password)
				Expect(err).NotTo(HaveOccurred())
			})
	...
	Context("TLS", func() {
		When("TLS is correctly configured", func() {
			...
			It("talks amqps with RabbitMQ", func() {
				...
				Expect(publishToNewQueueAMQPS(sentMessage, username, password, hostname, amqpsNodePort, caFilePath)).To(Succeed())
